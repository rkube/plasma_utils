...
class XGC_it_dataset_kneighbor(InMemoryDataset):
    ...
    def process(self):
        ...
        with adios2.open(join(self.root, fname_mesh), "r") as df:
            coords = df.read("/coordinates/values")
            nextnode = df.read("nextnode")
            nc = df.read("/cell_set[0]/node_connect_list")
            df.close()

        with adios2.open(join(self.root, fname_B), "r") as df:
            Bvec = df.read("/node_data[0]/values")
            df.close()
...
class XGC_it_dataset_kneighbor_norm2(InMemoryDataset):
    ...
    def process(self):
        ...
        graph_list = []
        toc = timeit.default_timer()
        print(f"Compiling networkx graph took {(toc-tic):6.4f}s")

        #######################################################################################
        # Construct a sub-graphs with specified depth from the global simulation domain
        #
        # Idea: use different approaches to build local neighborhoods
        tic = timeit.default_timer()
        for root_vtx in list(G.nodes):
            subgraph = nx.Graph()
            sub_edges = nx.algorithms.traversal.breadth_first_search.bfs_edges(G, source=root_vtx, depth_limit=self.depth)
        ...
        toc = timeit.default_timer()
        print(f"Compiling graph_list took {(toc - tic):6.4f}s")

        tic = timeit.default_timer()
        if self.pre_filter is not None:
            graph_list = [data for data in graph_list if self.pre_filter(data)]
        toc = timeit.default_timer()
        print(f"Applying pre-filter took {(toc - tic):6.4f}s")
...
class XGC_kneighbor_squashed(InMemoryDataset):
    ...
    def process(self):
        ...
        for root_vtx in all_vertices:
            ...
            graph_list.append(Data(edge_index=edge_index, edge_attr=torch.DoubleTensor(weights),
                                   x=data_x[neighbors, :], y=data_y[neighbors, :], root_vtx=root_vtx))
            toc = timeit.default_timer()
            tictoc_list.append(toc-tic)
        tictocs = np.array(tictoc_list)
        num_graphs = len(graph_list)

        print(f"Compiling {num_graphs:05d} graphs took {(tictocs.sum()):6.3f}s")
...
class XGC_squashed_noedges(InMemoryDataset):
    r"""Uses squashed graph neighborhoods where the root vertex connects directly
    to all vertices at max k edges away.
    ...
    def processed_file_names(self):
        print("Called processed files")
        return [f"xgc_kneighbor_squashed_noedges_depth{self.depth}_{self.idx_n:03d}{self.idx_k:02d}.pt"]
    ...
    def process(self):
        ...
        with np.load(join(self.root, "raw", "xgc.mesh.npz")) as df:
            coords = df["/coordinates/values"]
            nextnode = df["nextnode"]
            nc = df["/cell_set[0]/node_connect_list"]

        with np.load(join(self.root, "raw", "xgc.bfield.npz")) as df:
            Bvec = df["/node_data[0]/values"]
        ...
        Btotal = np.sqrt(np.sum(Bvec**2.0, axis=1))  # Total magnetic field, in T
        # Get background density and temperature. Hard-code those for test cases.
        ne0, ni0 = 1e19, 1e19 # Background electron/ion density, in m^-3
        Te0, Ti0 = 2e3, 2e3 # Background electron/ion temperaure, in eV
        ...
        # Do not transpose B
        features_node["B"] = np.tile(Btotal, (num_planes, 1)).flatten() / Btotal.max()
        # Do not transport R
        features_node["R"] = np.tile(coords[:, 0], (num_planes, 1)).flatten()
        # Do not transpose Z
        features_node["Z"] = np.tile(coords[:, 1], (num_planes, 1)).flatten()
        features_node["plane_idx"] = np.tile(np.arange(num_planes), (coords.shape[0], 1)).T.flatten()
        features_node["iter"] = np.ones_like(features_node["B"]) * self.idx_k

        # For statistics of ne, ni, ue, ui, apar and dpot fields over time, see explore_data_mldata_case2_normalizations.ipynb
        data_x = torch.tensor([features_node["iter"], features_node["B"],
                               features_node["R"], features_node["Z"], features_node["plane_idx"],
                               features_node["eden"] * 1e-7, features_node["iden"] * 1e-7,
                               features_node["u_e"] * 1e-14, features_node["u_i"] * 1e-14,
                               features_node["a_par"] * 1e13, features_node["dpot"] * 1e7,
                               features_node["apar_res"], features_node["pot_res"]]).T
        ...
        for root_vtx in all_vertices:
            ...
            graph_list.append(Data(edge_index=edge_index, x=data_x[neighbors, :], y=data_y[root_vtx, :], root_vtx=root_vtx))
            toc = timeit.default_timer()
            tictoc_list.append(toc-tic)
        tictocs = np.array(tictoc_list)
        num_graphs = len(graph_list)
